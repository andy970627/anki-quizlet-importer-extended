// Modified 2026-09-26 for the "Better Lyrics + 歌詞節奏模式" fork: starts the lyric rhythm mode
// (src/modules/rhythm/) and tears it down with the rest of the content script. See FORK.md.
...
import { initLyricRhythmMode } from "@modules/rhythm/connect";
import type { RhythmGameController } from "@modules/rhythm/rhythmGame";
 ...
 */
let rhythmMode: RhythmGameController | null = null;
...
async function modify(isDisposed: () => boolean): Promise<void> {
  ...
  setUpAvButtonListener();
  // Fork addition: the lyric rhythm mode (osu-style hit circles timed to the synced lyrics).
  rhythmMode = initLyricRhythmMode();
  logCore(
    INITIALIZE_LOG,
